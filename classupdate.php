...
$query1="SELECT `group_id` FROM `teacher_group` WHERE `teacher_id`='$id'";
$result = mysql_query($query1) or die(mysql_error());
			...
			$query1="SELECT `class_id`, `class_name` FROM `class` WHERE class_id IN (SELECT `class_id` FROM `teacher_class` WHERE teacher_id='$id')";
			$result=mysql_query($query1) or die(mysql_error());																	
			if(mysql_num_rows($result)){
				while($row=mysql_fetch_array($result)){							
					$classteacherid[]=$row['class_id'];
					
				}
			}
			
			foreach($group as $value3){
			$query2="SELECT `group_name` FROM `tbl_group` WHERE `group_id`=$value3";
			$result = mysql_query($query2) or die(mysql_error());
						if(mysql_num_rows($result)){
							while($rows=mysql_fetch_array($result, MYSQL_BOTH)){
								$groups[]=$rows['group_name'];
							}
						}
						}?>	
<table class="table table-bordered table-hover" border="1" >
			
				<form action="classsubmitupdate.php" method="get">
					<tr><th><h2>SELECT CLASSES<h2></th></tr>
			
<?php			foreach (array_keys($group) as $key){
				$query="SELECT class_name, class_id from class WHERE class_id IN (SELECT class_id from t_group_class WHERE group_id=$group[$key])";
				$result = mysql_query($query) or die(mysql_error());?>
					<tr>
						<td> &nbsp </td>
					</tr>
					<tr style="background-color:#f5f5f5;">
						<td><?php echo "GROUP : ".$groups[$key] ;?></td>
					</tr>
				<?php if(mysql_num_rows($result)){
				while($row=mysql_fetch_array($result, MYSQL_BOTH)){
					foreach($classteacherid as $value){
						if($row['class_id']==$value){
							$b="checked";
						}
					}
					...
					<tr>
						<td>
							<input type="checkbox" name="class[]" value="<?php print($row['class_id']); ?>" id="<?php print($row['class_name']); ?>" <?php echo $b; ?> /><?php print($row['class_name']); ?><br/>
						</td>
					</tr>
