		...
		$query5="SELECT class_id FROM teacher_class WHERE `teacher_id`='$id'";
			...
			?>
			 <table class="table table-bordered table-hover" border="1" >
				...
				<form action="subjectsubmitupdate.php" method="get">
					<tr><th><h2>SELECT SUBJECTS<h2></th></tr>
			
<?php
	$query11="SELECT `subject_id` FROM `subject` WHERE subject_id IN (SELECT `subject_id` FROM `teacher_subject` WHERE teacher_id='$id')";
			$result=mysql_query($query11) or die(mysql_error());																	
			if(mysql_num_rows($result)){
				while($row=mysql_fetch_array($result)){							
					$subjectteacherid[]=$row['subject_id'];
				}
			}
		...
		if(mysql_num_rows($result)){
			...
			$result2 = mysql_query($query2) or die(mysql_error());
				...
				while($row4=mysql_fetch_array($result4, MYSQL_BOTH)){
					foreach($subjectteacherid as $value){
						if($row4['subject_id']==$value){
							$b="checked";
						}
					}
					?>
					<tr>
						<td>
							<input type="checkbox" name="subject[]" value="<?php print($row4['subject_id']); ?>" id="<?php print($row4['subject_name']); ?>" <?php echo $b; ?> /><?php print($row4['subject_name']); ?><br/>
						</td>
					</tr>
